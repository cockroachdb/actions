...
func Run(
	...
	for attempt := 1; attempt <= cfg.MaxRetries; attempt++ {
		...
		if err != nil || resultText == "" {
			action.LogWarning(fmt.Sprintf("No result text extracted from Claude output on attempt %d", attempt))
			if data, readErr := os.ReadFile(outputFile); readErr == nil {
				action.LogWarning("Could not extract result from Claude output — check step logs for raw output")
				action.LogInfo(string(data))
			}
		} else {
	...
	action.SetOutput("implementation", implStatus)
...
func pushAndPR(
	...
	stagedDiff, err := gitClient.Diff("--quiet", "--cached")
	_ = stagedDiff
	if err == nil {
