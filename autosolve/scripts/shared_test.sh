...
expect_failure "validate_inputs: no prompt or skill" "At least one of" test_validate_no_input
...
expect_failure "validate_inputs: create_pr missing fork config" "fork_owner" test_validate_pr_missing
...
test_auth_none() { unset ANTHROPIC_API_KEY CLAUDE_CODE_USE_VERTEX CLAUDE_CODE_USE_BEDROCK 2>/dev/null; validate_auth; }
expect_failure "validate_auth: no auth" "No Claude authentication" test_auth_none

test_auth_api_key() { ANTHROPIC_API_KEY=sk-test; validate_auth; }
expect_success "validate_auth: api key" test_auth_api_key

test_auth_vertex() { unset ANTHROPIC_API_KEY 2>/dev/null; CLAUDE_CODE_USE_VERTEX=1; ANTHROPIC_VERTEX_PROJECT_ID=proj; CLOUD_ML_REGION=us-east5; validate_auth; }
expect_success "validate_auth: vertex" test_auth_vertex

test_auth_vertex_missing() { unset ANTHROPIC_API_KEY ANTHROPIC_VERTEX_PROJECT_ID 2>/dev/null; CLAUDE_CODE_USE_VERTEX=1; CLOUD_ML_REGION=us-east5; validate_auth; }
expect_failure "validate_auth: vertex missing project" "ANTHROPIC_VERTEX_PROJECT_ID" test_auth_vertex_missing

test_auth_bedrock() { unset ANTHROPIC_API_KEY CLAUDE_CODE_USE_VERTEX 2>/dev/null; CLAUDE_CODE_USE_BEDROCK=1; AWS_REGION=us-east-1; validate_auth; }
expect_success "validate_auth: bedrock" test_auth_bedrock

test_auth_bedrock_missing() { unset ANTHROPIC_API_KEY CLAUDE_CODE_USE_VERTEX AWS_REGION 2>/dev/null; CLAUDE_CODE_USE_BEDROCK=1; validate_auth; }
expect_failure "validate_auth: bedrock missing region" "AWS_REGION" test_auth_bedrock_missing
...
test_build_prompt_only() {
  ...
  build_prompt
  grep -q 'Fix the bug' "$PROMPT_FILE"
}
...
test_build_preamble() {
  ...
  INPUT_FOOTER_TYPE=implementation; INPUT_BLOCKED_PATHS='.github/workflows/'
  build_prompt
  grep -q 'system_instruction' "$PROMPT_FILE"
}
...
test_build_skill() {
  ...
  INPUT_FOOTER_TYPE=implementation; INPUT_BLOCKED_PATHS='.github/workflows/'
  build_prompt
  grep -q 'This is skill content' "$PROMPT_FILE"
}
...
expect_failure "build_prompt: missing skill file" "Skill file not found" test_build_missing_skill
...
test_build_assessment_footer() {
  ...
  build_prompt
  grep -q 'ASSESSMENT_RESULT' "$PROMPT_FILE"
}
...
test_build_custom_criteria() {
  ...
  build_prompt
  grep -q 'Only proceed if trivial' "$PROMPT_FILE"
}
...
test_build_additional() {
  ...
  INPUT_FOOTER_TYPE=implementation; INPUT_BLOCKED_PATHS='.github/workflows/'
  build_prompt
  grep -q 'Do not run tests' "$PROMPT_FILE"
}
...
test_extract_proceed() {
  ...
  extract_result "$RESULT_DIR/proceed.json" "ASSESSMENT_RESULT" | grep -q "ASSESSMENT_RESULT - PROCEED"
}
...
test_extract_skip() {
  ...
  extract_result "$RESULT_DIR/skip.json" "ASSESSMENT_RESULT" | grep -q "ASSESSMENT_RESULT - SKIP"
}
...
test_extract_success() {
  ...
  extract_result "$RESULT_DIR/success.json" "IMPLEMENTATION_RESULT" | grep -q "IMPLEMENTATION_RESULT - SUCCESS"
}
...
test_session_id() {
  ...
  extract_session_id "$RESULT_DIR/session.json" | grep -q "abc123"
}
