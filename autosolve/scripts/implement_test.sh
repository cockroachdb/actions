...
git init "$TMPDIR_TEST/repo" >/dev/null
cd "$TMPDIR_TEST/repo"
...
git add src/main.go
git commit -m "add safe file" >/dev/null
...
expect_failure_output "security_check: blocked staged file" "Blocked path modified" test_blocked_staged
...
expect_failure_output "security_check: blocked untracked file" "Blocked path" test_blocked_untracked
...
expect_failure_output "security_check: multiple blocked paths" "Blocked path" test_multiple_blocked
...
test_outputs_success() {
  ...
  check_contains "status=SUCCESS" "$GITHUB_OUTPUT"
}
...
test_outputs_pr_failed() {
  ...
  check_contains "status=FAILED" "$GITHUB_OUTPUT"
}
